...
def main():
    ...
    # NOTE: calling .predict() once per image (rather than stream=True on a whole
    # list, which ultralytics batches into a single oversized forward pass - it
    # broke the engine's static batch-1 shape and blew up the pt model's memory
    # use when tried) is the safe way to drive pt/onnx/engine identically. It does
    # add per-call Python/dataloader overhead on top of the model's own compute
    # (see wall_ms vs preprocess+inference+postprocess below) - that overhead is
    # a benchmark-loop artifact, not something a real continuous pipeline (e.g.
    # hydrakon_bringup's camera stream, which keeps one predictor/session alive
    # across frames) pays per frame, so we report both numbers explicitly.
    ...
    print(f"[{args.backend}] done: {len(sample)} images, "
          f"{len(sample) / wall_total_s:.2f} FPS (naive per-call wall), "
